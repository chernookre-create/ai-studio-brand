...
def _шрифт(*кандидаты):
    ...
    на маке (Ф183). Подписи на контактном листе русские, поэтому кириллица в шрифте
...
def build(run, src):
    ...
    # Ни одного кадра — это не сбой сборки, а сообщение человеку: скорее всего кадры ещё
    # не занесены в results/, и связать их не с чем. Прежде здесь падал np.vstack с
    # «need at least one array to concatenate» — трассировкой вместо объяснения, и упал бы
    # ровно на первом пилотном кадре, потому что у пилота записи в results/ ещё нет (Ф169).
    if not tiles:
        print(f'\nВ папке {src} нет кадров, которые связаны с записями в results/*.json.')
        print('Сдача собирает только посчитанные кадры: кадр без счёта человеку не показывается.')
        print('Занеси кадр в results/<серия>.json полем «код» и запусти сдачу снова.\n')
        return None
    while len(tiles) % 3:
    ...
    # Сдача не заканчивается архивом. Решение 19.08: кадры уезжают на Яндекс.Диск, и это
    # часть сдачи, а не отдельная просьба. Печатается здесь, потому что тот, кто запускает
    # сдачу, следующий шаг должен видеть, а не вспоминать: до 19.08 выгрузка была записана
    # в README и в «Состоянии», но не в инструкции, не в скиле и не здесь (Ф168).
    print('\nДальше — выгрузка на Яндекс.Диск, это часть сдачи:')
    print('  ~/Yandex.Disk.localized/Фотосессия ИИ/<АРТИКУЛ>/<NN>/')
    print('  Артикул называет заказчик — не выводится из названия вещи и не придумывается.')
    print('  Несколько съёмок одного артикула — в одной папке, номера по порядку.')
    print('  На Диск идут только кадры: набор, промпты и паспорта остаются в репозитории.')
    print('  Перенос через мост; сверь число отправленных и дошедших файлов.\n')
    return f'{out}.zip'
