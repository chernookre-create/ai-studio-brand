...
def main():
    ...
    # Описание пуговицы взято из паспорта изделия, а не из памяти: 18.08 выяснилось, что
    # шесть промптов и этот список глазами носили «гладкий купол в широком кольце с
    # насечками», потом «гранёный купол в узком витом ободке» — оба неверны. На слоте 3
    # кованое золото самородком: гранёная середина и неровный битый ободок (Ф144, Ф171).
    print('Далее глазами: три пуговицы и ни одной больше, рисунок пуговицы — КОВАНОЕ ЗОЛОТО')
    print('самородком: гранёная середина, неровный битый ободок; НЕ гладкий купол и НЕ витой')
    print('ободок — как в паспорте изделия (папка refs/approved). V-вырез, нет полосы')
    print('по низу и манжетам, низ перекрывает ПОЯС джинсов: пояса, шлёвок и пуговицы джинсов')
    print('не видно вовсе; карманов на кардигане нет.\n')
...
# подсадка проверки: ссылка на Ф999
