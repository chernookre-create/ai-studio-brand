...
def main():
    ...
    # .venv — тот же класс, что __pycache__: окружение замеров ставится
    # automation/setup.sh рядом с комплектом и даёт 16 тысяч файлов вместо двух сотен.
    # Оно в .gitignore, комплектом не является и в счёт идти не должно (19.08).
    ПРОПУСК = ('.git', '__pycache__', '.pytest_cache', '.venv')
    ...
    пункт(f'3. Девять слотов набора — серия «{series}»' if slots else '3. Девять слотов набора')
    if slots is None:
    ...
    if not os.path.exists(рп):
        ...
        # Живые пункты берутся из исходника, а не из того, что уже напечатано: пункт 7
        # печатается ПОСЛЕ этого, и сверка «по напечатанному» объявляла его несуществующим.
        # Порядок печати — не свойство пункта, а случайность его места в файле.
        свой = open(os.path.join(TOOLS, 'selftest.py'), encoding='utf-8').read()
        живые = set(_re4.findall(r"пункт\(f?'(\d+[а-яё]?)\.", свой))
        живые |= {'5б'}          # печатает rules_selftest.py, гоняемый пунктом 5б
    ...
    пункт('6о. Каждый номер находки, упомянутый в комплекте, есть в реестре')
    # Обратное направление к 6н. Тот сверяет ссылки реестра на пункты отчёта; здесь —
    # ссылки комплекта на находки. Ф177 записала три таких номера (Ф168, Ф169, Ф170):
    # они цитировались комментариями и не были определены нигде, и это назвали рецидивом
    # Ф94. Проверки не завели, сославшись на то, что журнал лежит вне репозитория. Но
    # чтобы заметить ссылку в пустоту, журнал не нужен — достаточно реестра. 19.08 таких
    # номеров нашлось тринадцать, вчетверо больше записанного.
    #
    # Номер занят с момента появления в любом файле, а не с записи в журнал: пока номер
    # цитируется, но не заведён, его может занять следующая находка — и две разные вещи
    # станут называться одинаково.
    _нах2 = {}
    _рег = os.path.join(ROOT, '00_obs', 'находки.json')
    if os.path.exists(_рег):
        _нах2 = json.load(open(_рег, encoding='utf-8')).get('находки', {})
    if not _нах2:
        check('реестра находок нет — сверять не с чем', False, _рег)
    else:
        _упом = {}
        for base, dirs, files in os.walk(ROOT):
            ...
            for f in files:
                if not f.endswith(('.py', '.md')):
                    continue
                # Журнал — источник определений, а не ссылок на них. Внутри него номера
                # стоят в прозе, и часть заголовков сцеплена так, что разбор их не
                # заводит. Считать эти упоминания ссылками в пустоту — значит смешать
                # три разные вещи: честно свободный номер, запись в форме, которую не
                # берёт разбор, и настоящую ссылку в никуда. Проверка от этого краснеет
                # всегда и перестаёт читаться. Здесь судим только комплект.
                #
                # Номера примерами в комментариях не приводятся: номер занят с момента
                # появления в любом файле, и пример занял бы чужой.
                if f == 'журнал.md':
                    continue
                путь = os.path.join(base, f)
                try:
                    текст = open(путь, encoding='utf-8').read()
                except (OSError, UnicodeDecodeError):
                    continue
                for m in _re4.finditer(r'Ф(\d+)(?!\d)', текст):
                    _упом.setdefault('Ф' + m.group(1), set()).add(os.path.relpath(путь, ROOT))
        нет_в_реестре = sorted((n for n in _упом if n not in _нах2),
                               key=lambda x: int(x[1:]))
        деталь = '; '.join(f'{n} ({", ".join(sorted(_упом[n]))})' for n in нет_в_реестре[:6])
        check(f'{len(_упом)} номеров упомянуто в комплекте, все заведены',
              not нет_в_реестре, деталь)
