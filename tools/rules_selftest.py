...
def _папка_изделия():
    """Папка промптов сегодняшнего изделия — из данных съёмки, а не литералом.

    Проверка 6б немедленно поймала литерал `prompts/кардиган-шоколад-горошек` в этом файле:
    со следующим изделием образец фото указывал бы в чужую папку (закон E5).
    """
    import json
    путь = os.path.join(ROOT, 'refs', 'CURRENT.json')
    try:
        return json.load(open(путь, encoding='utf-8')).get('папка_промптов') or ''
    except Exception:                                            # noqa: BLE001
        return ''


def _первый_рабочий(папка):
    """Любой рабочий промпт изделия: они все зелёные, для поломок годится любой."""
    корень = os.path.join(ROOT, папка)
    if not os.path.isdir(корень):
        return ''
    файлы = sorted(f for f in os.listdir(корень) if f.endswith('.txt'))
    return os.path.join(папка, файлы[0]) if файлы else ''


ОБРАЗЦЫ = {
    'фото':     _первый_рабочий(_папка_изделия()),
...
def main():
    ...
    # Считаем уникальные правила, а не проверки. A1 общее для четырёх классов и ломается в
    # каждом — но правило от этого одно. Прежняя строка печатала 57 и это число разъехалось
    # по документам, где рядом стояло «35 из 53» с другим знаменателем (Ф163).
    # Коды тоже нельзя складывать в множество: `B1` в ФОТО и `B1` в ИНТЕРЬЕРЕ — разные правила
    # (закон F9 про пространства кодов). Уникальны правила, а не коды: сумма по классам плюс
    # одно общее.
    уник = (len(C.CHECKS_PHOTO) + len(C.CHECKS_INTERIOR)
            + len(C.CHECKS_PACKSHOT) + len(C.CHECKS_VIDEO) + 1)
    print(f'Правил уникальных: {уник} — по классам {len(C.CHECKS_PHOTO)} ФОТО, '
          f'{len(C.CHECKS_INTERIOR)} ИНТЕРЬЕР, {len(C.CHECKS_PACKSHOT)} ПЭКШОТ, '
          f'{len(C.CHECKS_VIDEO)} ВИДЕО и одно общее A1.')
    print(f'Проверок сделано: {всего} — общее правило ломается в каждом классе отдельно.')
    for b in беды:
        print('  СБОЙ  ' + b)
    if беды:
